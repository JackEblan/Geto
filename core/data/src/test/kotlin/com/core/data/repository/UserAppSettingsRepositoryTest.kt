...
class UserAppSettingsRepositoryTest {
    ...
    @Test
    fun `get UserAppSettings upsert UserAppSettingsItemEntity with enabled property returns not empty`() =
        runTest {
            val entity = UserAppSettingsItemEntity(
                enabled = true,
                settingsType = SettingsType.SYSTEM,
            ...
            assertContains(
                iterable = userAppSettingsDao.entitiesStateFlow.first(), element = entity
            )
        }
    ...
    fun `get UserAppSettings filter by package name returns not empty`() = runTest {
        ...
        assertContains(
            iterable = userAppSettingsDao.getUserAppSettingsList(entity.packageName).first(),
            element = entity
        )
    }
}